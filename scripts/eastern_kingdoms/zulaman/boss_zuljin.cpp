...
SDName: Boss_Zuljin
SD%Complete: 0
SDComment: ( base script by Xfurry corrections and ect by fallenangelX)
SDCategory: Zul'Aman 
EndScriptData */
...
struct MANGOS_DLL_DECL boss_zuljinAI : public ScriptedAI
{
    ...
    std::list<ObjectGuid*> m_lSummonsGUIDList;
    ...
    void SpawnAdds()
    {
        ...
        for(uint8 i = 0; i < 4; i++)
        {
            ...
            if(pCreature)
            {
                ...
                SpiritGUID[i] = pCreature->GetObjectGuid();
            }
        }
    }
    ...
    /*void DespawnSummons()
    {
        ...
        {
            for(std::list<ObjectGuid*>::iterator itr = m_lSummonsGUIDList.begin(); itr != m_lSummonsGUIDList.end(); ++itr)
                if (Creature* pTemp = m_creature->GetMap()->GetUnit(*itr))
                    pTemp->ForcedDespawn();
        }
    }*/
            ...
            if(NextPhase == 2)
            {
                ...
                for(uint8 i = 0; i < 4; i++)
                {
                    ...
                    if(Vortex)
                    {
                        ...
                        //m_lSummonsGUIDList.push_back(Vortex->GetObjectGuid());
                    }
                }
            }
            else
        ...
        case 3:
            if(Claw_Rage_Timer <= diff)
            {
                if(!TankGUID)
                {
                    if(Unit* target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
                    {
                        TankGUID = m_creature->getVictim()->GetObjectGuid();
